...
import { PrismaClient, StudyFrequency, PlanStatus } from '@prisma/client';
...
class StudyPlanService {
  ...
  async generateFragments(
    ...
    for (let i = 0; i < contentFragments.length; i++) {
      ...
      fragments.push({
        fragmentNumber: i + 1,
        content: contentFragments[i],
        summary: '', // Will be filled by AI service
        questions: [], // Will be filled by AI service
        scheduledDate
      });
    }

    // Create fragments in database
    const createdFragments = await prisma.studyFragment.createMany({
  ...
  calculateScheduledDate(startDate: Date, sessionIndex: number, frequency: StudyFrequency): Date {
    ...
    switch (frequency) {
      case StudyFrequency.DAILY:
        ...
        break;
      case StudyFrequency.EVERY_OTHER_DAY:
