...
interface StudyPlanFormProps {
  ...
  isLoading?: boolean;
  mode?: 'full' | 'fromMaterial';
}

export function StudyPlanForm({ onSubmit, isLoading = false, mode = 'full' }: StudyPlanFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!studyFrequency || !duration || !startDate) {
      return;
    }

    if (isFullMode && (!title || !group || !level)) {
      return;
    }
